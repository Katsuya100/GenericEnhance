...
{
    public struct NotEqual<T, TX, TY> :
        ITypeFormula<bool>
        where TX : struct, ITypeFormula<T>
        where TY : struct, ITypeFormula<T>
    {
        public static readonly bool ResultValue = ConditionalLogicalUtils.NotEqual(TypeFormula.GetValue<TX, T>(), TypeFormula.GetValue<TY, T>());
